...
public class CollectionManager {
		...
		if(numParameter == 5) {
			//check if the date is valid
			 if(new Date(arr[4]).isValid()) {
				 ...
				 Genre genre = validateGenre(arr[3]);
				 Album a = new Album(arr[1], arr[2], genre, new Date(arr[4]));
				 if(collection.add(a)) {
	 ...
	 */
	private Genre validateGenre(String g) {
		 Genre retGen = Genre.Unknown;
		 String[] genreStrings = {"Classical", "Country", "Jazz", "Pop", "Unknown"};
		 for(String x : genreStrings) 
		 	{
			 if(capitalize(g).equals(x)) {
				 retGen = Genre.valueOf(x);
			}
		 }
		 return retGen;
	}
	...
	public void run() {
			...
			while(!(command.equals("Q")))
			{
				...
				if(retMes.length() > 0) System.out.println(retMes);
			}
			in.close();
	}
